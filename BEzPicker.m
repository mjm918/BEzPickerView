...
@class BEzPickerViewLayout;

@protocol BEzPickerViewLayoutDelegate <NSObject>
- (BEzPickerViewStyle)pickerViewStyleForCollectionViewLayout:(BEzPickerViewLayout *)layout;
@end

@interface BEzPickerViewCell : UICollectionViewCell
...
@interface BEzPickerViewLayout : UICollectionViewFlowLayout
@property (nonatomic, assign) id <BEzPickerViewLayoutDelegate> delegate;
@end

@interface BEzPickerViewDelegateIntercepter : NSObject <UICollectionViewDelegate>
@property (nonatomic, weak) BEzPicker *pickerView;
@property (nonatomic, weak) id <UIScrollViewDelegate> delegate;
@end

@interface BEzPicker () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, BEzPickerViewLayoutDelegate>
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, assign) NSUInteger selectedItem;
@property (nonatomic, strong) BEzPickerViewDelegateIntercepter *intercepter;
- (CGFloat)offsetForItem:(NSUInteger)item;
- (void)didEndScrolling;
- (CGSize)sizeForString:(NSString *)string;
@end

@implementation BEzPicker
- (void)initialize
{
    ...
    self.pickerViewStyle = self.pickerViewStyle ?: BEzPickerViewStyle3D;
    ...
    [self.collectionView registerClass:[BEzPickerViewCell class]
            forCellWithReuseIdentifier:NSStringFromClass([BEzPickerViewCell class])];
    [self addSubview:self.collectionView];
    
    self.intercepter = [BEzPickerViewDelegateIntercepter new];
...
- (void)setDelegate:(id<BEzPickerDelegate>)delegate
{
...
- (BEzPickerViewLayout *)collectionViewLayout
{
    BEzPickerViewLayout *layout = [BEzPickerViewLayout new];
        ...
        case BEzPickerViewStyle3D: {
            if ([self.dataSource numberOfItemsInPickerView:self]) {
                for (NSUInteger i = 0; i < [self collectionView:self.collectionView numberOfItemsInSection:0]; i++) {
                    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:i inSection:0];
                    BEzPickerViewCell *cell = (BEzPickerViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
...
{
    BEzPickerViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([BEzPickerViewCell class])
...
@implementation BEzPickerViewCell
...
@interface BEzPickerViewLayout ()
@property (nonatomic, assign) CGFloat width;
@property (nonatomic, assign) CGFloat midX;
@property (nonatomic, assign) CGFloat maxAngle;
@end

@implementation BEzPickerViewLayout
...
@implementation BEzPickerViewDelegateIntercepter
